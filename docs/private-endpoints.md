...
## Getting Started 
This setup expects that customers already have an Azure FHIR Service (Azure API for FHIR) along with FHIR-Proxy up and running. This setup will support additional components that work with VNets and VNet Peering.
...
At this point you should be able to test your private endpoint using either your VM with Postman - or - a PC connected to the VPN Gateway (above). Connections from the VNet should work, and connections outside the VNet should fail.
...
_Note: The private DNS update may fail due to dynamic IP's on the function_ - see the sample error below.
```json
  {
    "status": "Failed",
    "error": {
...
In the search bar, type in "key vault".
