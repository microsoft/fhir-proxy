...
#
#FHIR Proxy Setup for OIDC --- Author Steve Ordahl Principal Architect Health Data Platform
#
...
declare fpoidciss=""
declare fpoidcsecretkey=""
declare fpoidctokenidclaim=""
...
#OIDC Parameters
echo "Enter the Open Id Connect Issuer URL:"
		read fpoidciss
		if [ -z "$fpoidciss" ] ; then
			echo "You must provide a  Open Id Connect Issuer URL"
			exit 1;
		fi
echo "Enter the valid audiences for your Open Id Connect Issuer (seperated by commas ','):"
		read fpoidcvalidaudiences
		if [ -z "$fpoidcvalidaudiences" ] ; then
			echo "You must provide the valid audiences for your Open Id Connect Issuer"
			exit 1;
		fi
echo "Enter valid issuers Open Id Connect configuration (seperated by commas ',') ["$fpoidciss"]:"
	  read fpoidcvalidissuers
...
(
	...
	echo "Your Open Id Connect Provider is: "$fpoidciss
	echo " "
	echo "Next Steps:  "
	echo " 1) You can register SMART compliant client applications with your OIDC provider to provide appropriate scopes/context for launching and access"
	echo " "
