...
(
		...
		stepresult=$(az appservice plan create -g  $resourceGroupName -n $deployprefix$serviceplanSuffix --sku F1 --tags $tags)
		#Create the function app
		echo "Creating Secure FHIR Proxy Function App ["$faname"]..."
		fahost=$(az functionapp create --name $faname --storage-account $deployprefix$storageAccountNameSuffix --resource-group $resourceGroupName --consumption-plan-location eastus --runtime dotnet --os-type Windows --functions-version 3 --tags $tags --query defaultHostName --output tsv)
